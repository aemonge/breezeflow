...
- `--during <command>`: Command to run concurrently with the main command.
- `--on-succeed <command>`: Command to run if the main command succeeds.
- `--on-fail <command>`: Command to run if the main command fails.
- `--delay <seconds>`: (default: 1) Delay between retry attempts for during, 
    on-succeed, and on-fail.
- `--max-attempts <number>`: (default: 3) Maximum retry attempts for during, 
    on-succeed, and on-fail.
...
```{bash}
breezeflow 'sleep 2' --during 'echo "Main command is running..."'
```

2. Simulate success with a callback:

```{bash}
breezeflow 'sleep 2 && true' \
